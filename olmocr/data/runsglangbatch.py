...
class SGLangClient:
    ...
    def create_batch(self, file_id, metadata=None):
        ...
        response = requests.post(f"{self.base_url}/batch", json=data)
        response.raise_for_status()
        return response.json()["batch_id"]

    def get_batch_status(self, batch_id):
        response = requests.get(f"{self.base_url}/batch/{batch_id}/status")
        response.raise_for_status()
        return response.json()

    def download_result(self, batch_id):
        response = requests.get(f"{self.base_url}/batch/{batch_id}/result")
        response.raise_for_status()
...
client = SGLangClient("http://localhost:3000")


def upload_and_start_batch(file_path):
    print(f"Uploading {file_path} to sglang...")
    file_id = client.upload_file(file_path)
    print(f"File uploaded successfully: {file_id}")

    print(f"Creating batch job for {file_path}...")
    batch_id = client.create_batch(file_id, metadata={"description": "pdf gold/silver data"})
...
def download_batch_result(batch_id, output_folder):
    ...
    if status["state"] != "completed":
        print(f"WARNING: {batch_id} is not completed, status: {status['state']}")
        return batch_id, False

    print(f"Downloading batch data for {batch_id}")
    result_data = client.download_result(batch_id)
