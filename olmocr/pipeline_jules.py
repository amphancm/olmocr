...
async def process_page(args, worker_id: int, pdf_orig_path: str, pdf_local_path: str, page_num: int) -> PageResult:
    ...
    while attempt < MAX_RETRIES:
        ...
        try:
            ...
            if base_response_data and "choices" in base_response_data and base_response_data["choices"] and "message" in base_response_data["choices"][0] and "content" in base_response_data["choices"][0]["message"]:
                problematic_json_string = base_response_data["choices"][0]["message"]["content"]
                logger.debug(f"Problematic JSON string for {pdf_orig_path}-{page_num}: {problematic_json_string}")
